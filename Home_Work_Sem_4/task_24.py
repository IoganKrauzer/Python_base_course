...
def answer_out(list_1, b_num):                   # Для вывода итогого ответа
    if b_num == len(list_1):
        print(f"С куста {b_num } и рядом стоящих кустов вы соберете => ",
              list_1[0] + list_1[-2] + list_1[-1], "ягод")
    else:
        print(f"С куста {b_num } и рядом стоящих кустов вы соберете => ",
...
print("Количество ягод на кустах: " + ", ".join(map(str, l_1))) # некоторым кустом заданной во входном списке урожайности грядки.
print(bush_num)
answer_out(l_1, bush_num)                                    
