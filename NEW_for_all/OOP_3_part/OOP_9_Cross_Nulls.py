...
class TicTacToe:
    ...
    def __update_win_status(self):
        ...
        for i in self.pole:
            ...
            if all(x.value == self.COMPUTER_O for x in (row[i] for row in self.pole)):
                ...
                return
            #  по диагоналям
        if all(self.pole[i][i] == self.HUMAN_X for i in range(self._size)) or \
            all(self.pole[i][-1 -i] == self.HUMAN_X for i in range(self._size)):
            self._win = 1
            return

        if all(self.pole[i][i] == self.COMPUTER_O for i in range(self._size)) or \
            all(self.pole[i][-1 -i] == self.COMPUTER_O for i in range(self._size)):
            self._win = 2
            return
        #   на ничью
    ...
    def human_go(self):
        ...
        while True:
            ...
            if not(0 <= i <self.size) or not(0 <= j < self._size):
                continue
